...
COPYSELECT="COPY (select 'https://www.openstreetmap.org/node/'||osm_id as aaa_uri, ST_AsText(way) as geom, ST_X(way) as longitude, ST_Y(way) as latitude, * from planet_osm_point) TO '${BASEDIR}/planet_osm_point.csv' DELIMITER ',' CSV HEADER;"
psql -d osm-btn -c "${COPYSELECT}"

COPYSELECT="COPY (select 'https://www.openstreetmap.org/way/'||osm_id as aaa_uri, ST_AsText(way) as geom, ST_X(ST_Centroid(way)) as longitude, ST_Y(ST_Centroid(way)) as latitude, * from planet_osm_polygon) TO '${BASEDIR}/planet_osm_polygon.csv' DELIMITER ',' CSV HEADER;"
psql -d osm-btn -c "${COPYSELECT}"

COPYSELECT="COPY (select 'https://www.openstreetmap.org/way/'||osm_id as aaa_uri, ST_AsText(way) as geom, ST_X(ST_Centroid(way)) as longitude, ST_Y(ST_Centroid(way)) as latitude, * from planet_osm_line) TO '${BASEDIR}/planet_osm_line.csv' DELIMITER ',' CSV HEADER;"
psql -d osm-btn -c "${COPYSELECT}"
